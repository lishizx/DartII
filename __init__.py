...
from Ui_DantOutProt import Ui_DantOutProt #外箱条码生成
from Ui_OutCode import Ui_OutCode #扫描条码
from Ui_OutProt import Ui_OutProt #出货标签打印
...
class Dataset(QWidget, Ui_Dataset):
    ...
    def checklink(self):
        ...
        if isok == True:
            ...
            self.timer2.timeout.connect(self.hidewrap)
...
class UserLogin(QWidget, Ui_Login):
    def __init__(self):
        ...
        self.username.returnPressed.connect(self.nextinput)
...
class DantCode(QWidget, Ui_DantCode, BoxSing):
    ...
    def __init__(self,parent=None):
        ...
        self.tab5.clicked.connect(self.meau5)
        ...
        #加载已存在路径
        file_path = self.node.get("fileurl","urlcode")
        if file_path!="":            
    ...
    def CreatTable(self):
        ms = MSSQL()
        data = ms.get("select top 100 id,code,codetitle,datetimes from code where isuse='false' order by id desc")
        if data!=():
            data = list(data)
        else:
            data = []
    ...
    def choosefile(self):
        ...
        if dlg.exec_():
            ...
            self.GetBtwList(r''+file_path)
            self.node.set("fileurl","urlcode",file_path)
            print(file_path)
    ...
    def meau4(self):
        ...
        if self.root!=2:            
            self.hide()
    def meau5(self):
        self.SonClick.emit("5")
        if self.root!=1:            
            self.hide()

#加载出货扫描界面
class DantOutProt(QWidget, Ui_DantOutProt, BoxSing):
    SonClick = pyqtSignal(str)  
    def __init__(self,parent=None):
        super(DantOutProt, self).__init__(parent)
        self.setupUi(self)
        self.node = Node()
        self.ms = MSSQL()
        self.username = self.node.get("userinfo","username")
    ...
    def CreatTable(self):
        data = self.ms.get("select top 100 outproduct,addtime,cartonno,prname,pinno,mode,hole,outdate,optname,isgo from outproduct where 1=1 order by addtime desc")
        #@print(data)
        if data!=():
            data = list(data)
        else:
            data = []
        self.codelist.setRowCount(len(data))
        self.codelist.setColumnCount(10)#设置列    
        self.codelist.setHorizontalHeaderLabels(['出货码','生成时间','外箱条码','产品名称','产品代号','模号','穴号','有效时间','操作人员','确认状态'])
        for i in range(len(data)):
            for n in range(len(data[i])):
                datas = str(data[i][n])
                ...
                if n==1:       
                    ...
                    newItem.setTextAlignment(Qt.AlignCenter)
                    self.codelist.setItem(i,1, newItem)
                    self.codelist.setColumnWidth(1,160)
                ...
                if n==3:
                    ...
                    newItem.setTextAlignment(Qt.AlignCenter)
                    self.codelist.setItem(i,3, newItem)
                    self.codelist.setColumnWidth(3,100)
                if n==4:
                    newItem = QTableWidgetItem(datas)
                    newItem.setTextAlignment(Qt.AlignCenter)
                    self.codelist.setItem(i,4, newItem)
                    self.codelist.setColumnWidth(4,80)
                if n==5:
                    newItem = QTableWidgetItem(datas)
                    newItem.setTextAlignment(Qt.AlignCenter)
                    self.codelist.setItem(i,5, newItem)
                    self.codelist.setColumnWidth(5,80)
                if n==6:
                    newItem = QTableWidgetItem(datas)
                    newItem.setTextAlignment(Qt.AlignCenter)
                    self.codelist.setItem(i,6, newItem)
                    self.codelist.setColumnWidth(6,80)
                    
                if n==7:
                    newItem = QTableWidgetItem(datas)
                    newItem.setTextAlignment(Qt.AlignCenter)
                    self.codelist.setItem(i,7, newItem)
                    self.codelist.setColumnWidth(7,80)
                if n==8:                   
                    newItem = QTableWidgetItem(datas)
                    newItem.setTextAlignment(Qt.AlignCenter)
                    self.codelist.setItem(i,8, newItem)
                    self.codelist.setColumnWidth(8,80)    
                if n==9:
                    if datas=="false":
                        datas="未出货"
                    else:
                    ...
                    newItem = QTableWidgetItem(datas)
                    newItem.setTextAlignment(Qt.AlignCenter)
                    self.codelist.setItem(i,9, newItem)
                    self.codelist.setColumnWidth(9,80) 
    def push2(self):    
...
class OutCode(QWidget, Ui_OutCode, BoxSing):
    ...
    def push1(self):
        ...
        if self.outcode.text()=="":
            ...
            sql1 = "select ischecked,count,mode,hole,project from outcode where carton ='"+self.outcode.text()+"'"
            ...
            if len(data)!=0:     
                ...
                if data[0]=='false':
                    ...
                    self.mode = data[2]
                    if len(self.mode)==1:
                        self.mode ="00" + self.mode
                    elif len(self.mode)==2:
                        self.mode ="0" + self.mode
                    self.hole = str(int(data[3]))
                    self.project = data[4]
    ...
    def push2(self):
        if len(self.outcode.text())!=0:            
            if len(self.code.text())!=34:
                ...
                if nownum<allcount:            
                    self.nownum.setText(str(nownum))
                    sql1 = "select picbate from code where code ='"+self.code.text()+"' and isuse = 'true'  and mode='"+self.mode+"' and hole='"+self.hole+"' and project='"+self.project+"'"
                    print(sql1)
                    data = list(self.ms.get(sql1))
                    if data!=[]:
                        ...
                        if data2==[]:
                            insql =  "INSERT INTO getout(code, datetimes, creatuser,isout,cartonno,picbate) VALUES"
                            insql += "('"+code+"',(getdate()),'"+creatuser+"','"+isout+"','"+self.outcode.text()+"','"+picbate+"')"
                            self.ms.run(insql)
...
class OutProt(QWidget, Ui_OutProt, BoxSing):
    ...
    def __init__(self,parent=None):
        ...
        self.loadings()
        #加载已存在路径
        file_path = self.node.get("fileurl","urlprod")
        if file_path!="":            
            self.GetBtwList(r''+file_path)
       # print("是我")
    ...
    def loadings(self):
        ...
        self.outcode.returnPressed.connect(self.push1)
        self.chanpinno.currentIndexChanged.connect(self.Getpinno)
        self.shuaxin.clicked.connect(self.CreatTable)
        self.choosurl.clicked.connect(self.choosefile)
        self.created.clicked.connect(self.SaveConfig)
        self.bulkprint.clicked.connect(self.PrintCode)
        ...
        years = strftime('%Y',time.localtime(time.time()))
        now = datetime.datetime.strptime(now2,'%Y/%m/%d')
        noweek = strftime('%W',time.localtime(time.time()))
        self.zhoubieno.setText(years[2:]+noweek)
        self.dateEdit.setDate(now)
        
        sql = "select id,prname from prname"
        chanlist = list(self.ms.get(sql))
        self.chanpinno.addItem("请选择--")
        if chanlist!=[]:
            for item in chanlist:
                self.chanpinno.addItem(item[1])
    ...
    def PrintCode(self):                
        ...
        if self.comboBox.currentText()!='':
            ...
            if isdelete == True:
                #是否已经添加
                isinsert = self.ms.run(self.Dis)#打印条码的数据
                if isinsert == True: 
                    currentprinter = win32print.GetDefaultPrinter()
                    win32api.ShellExecute(0, "print", pmode, '/d:"%s"' % currentprinter, ".", 0)
    ...
    def push1(self):
        ...
        if datas!=[]:
            smql  = "select top 1 picbate from getout where cartonno='"+carton+"'"
            smql2 = list(self.ms.get(smql))
            
            #版本号
            if smql2!=[]:
                smql2 = smql2[0][0]
                self.banbenno.setText(smql2)
            datas = datas[0]
            #客户料号
            ...
            datas2 = list(self.ms.get("select prname,pinno,outdate,liushuino,outproduct from outproduct where cartonno='"+carton+"'"))
            if datas2==[]:
                self.outcode.setStyleSheet("color:#000;font-size:18px;padding-left:10px")
                self.tip.setText("外箱条码")
                self.tip.setStyleSheet("color:#000;font-size:16px;")
                self.RushOutProno()
            else:
                self.outcode.setStyleSheet("color:red;font-size:18px;padding-left:10px")
                self.tip.setText("已存在记录！")
                self.tip.setStyleSheet("color:red;font-size:16px;")
                self.RushOutProno2(datas2[0])
            
        else:
            self.Alert("条码未使用或条码不存在！")
             
    def Getpinno(self):
        currentText = self.chanpinno.currentText()
        if self.chanpinno.currentText()!="请选择--":
            curText = list(self.ms.get("select pno from prname where prname='"+currentText+"'"))[0][0]
            self.pin.setText(curText)
        else:
            self.pin.setText("0")
    ...
    def SaveConfig(self):
        if self.pin.text()=="0":
            self.Alert("请选择产品名称")
            return False
        cksql = "select count(*) from outproduct where cartonno='"+self.outcode.text()+"' or outproduct='"+self.outproduct.text()+"'"
        print(cksql)
        ckrel = list(self.ms.get(cksql))[0][0]
        if ckrel==0:
            sql = "insert into outproduct(outproduct,isgo,cartonno,prname,pinno,gudingno,outdate,mode,hole,liushuino,optname)values "
            nowdate = self.get_date()
            sql = sql+"('"+self.outproduct.text()+"','false','"+self.outcode.text()+"','"+self.chanpinno.currentText()+"'"
            sql = sql+",'"+self.pin.text()+"','"+self.gudingno.text()+"','"+nowdate+"','"+self.mode.text()+"','"+self.hole.text()+"','"+self.liushuino.text()+"','"+self.username+"')"
    ...
    #自动匹配生成出货码
    def RushOutProno(self):
        gudingno = self.gudingno.text()
        nowdate = self.get_date()[2:]
        pin = self.pin.text()
        mode = self.mode.text()
        hole = self.hole.text()
        #=================流水号生成开始=====================
        watersql = "select count(*) from outproduct where gudingno='"+gudingno+"' and outdate='"+nowdate+"' and "
        watersql+= "mode = '"+mode+"' and hole='"+hole+"' and pinno='"+pin+"'"
        waterel = list(self.ms.get(watersql))[0][0]
        waterel = self.ChangeNum(int(waterel)+1)
        if len(waterel)==1:
            waterel = "000"+waterel
        elif len(waterel)==2:
            waterel = "00"+waterel
            ...
            waterel = "0"+waterel
        self.liushuino.setText(waterel)      
        
        if len(mode)==2:
            waterel = waterel[1:]
        outproduct = gudingno+nowdate+pin+mode+hole+waterel
        #====================流水号生成结束==================
        cksql = "select count(*) from outproduct where cartonno='"+self.outcode.text()+"'"
        ckrel = list(self.ms.get(cksql))[0][0]
        if ckrel==0:
            self.outproduct.setText(outproduct)
            self.tip_4.setText(str(len(outproduct)))
    def RushOutProno2(self,data2):
        #print("数据流反向输出:")
        #print(data2)
        cpcount = self.chanpinno.count()
        for i in range(cpcount):
            if self.chanpinno.itemText(i)==data2[0]:
                self.chanpinno.setCurrentIndex(i)
        dates = data2[2]
        detester ="20"+ dates[0:2]+"-"+dates[2:4]+"-"+dates[4:6]
        date = datetime.datetime.strptime(detester,'%Y-%m-%d')
        self.dateEdit.setDate(date)
        self.liushuino.setText(data2[3])
        self.outproduct.setText(data2[4])
        self.tip_4.setText(str(len(self.outproduct.text())))
        #print(data2)
    def PutOutCode2(self):
        now2 = strftime('%Y/%m/%d',time.localtime(time.time()))
        years = strftime('%Y',time.localtime(time.time()))
        month = strftime('%m',time.localtime(time.time()))
        day = strftime('%d',time.localtime(time.time()))
        nowdate = years+month+day
        gudingno = self.gudingno.text()
        youxiao = self.get_date()
        pin = self.pin.text()
        mode = self.mode.text()
        hole = self.hole.text()
        ...
        self.outproduct.setText(outproduct)
        self.tip_4.setText(str(len(outproduct)))
        sql = "insert into printprod(metaid,meixiangno,mode,hole,banbenno,chanpinno,pin,gudingno,youxiao,pihaono,zhoubieno,yuanchanno,liushuino,outproduct,nowdate)values"
        sql = sql+"('"+self.metaid.text()+"','"+self.meixiangno.text()+"','TD"+mode+"','"+self.hole.text()+"#','"+self.banbenno.text()+"','"+self.chanpinno.currentText()
        sql = sql+"','"+self.pin.text()+"','"+self.gudingno.text()+"','"+youxiao+"','"+self.pihaono.text()+"','"+self.zhoubieno.text()+"','"+self.yuanchanno.text()
        sql = sql+"','"+self.liushuino.text()+"','"+outproduct+"','"+nowdate+"')"
        self.Dis = sql
        self.node .set("product", "gudingno", self.gudingno.text())
        self.node .set("product", "yuanchanno", self.yuanchanno.text())
        self.node .set("product", "pihaono", self.pihaono.text())
    def CheckNull(self):
        ...
        if self.pin.text()=="":
            self.Alert("产品代号未填！")
            isnull = True
        if self.gudingno.text()=="":
            self.Alert("固定编码未填！")
            isnull = True
            
            
        if self.pihaono.text()=="":
            self.Alert("批次号未填！")
            isnull = True
        if self.zhoubieno.text()=="":
            self.Alert("周别未填！")
            isnull = True
        if self.yuanchanno.text()=="":
            self.Alert("原产地未填！")
            isnull = True
        if self.liushuino.text()=="":
            self.Alert("流水号未填！")
            isnull = True
        return isnull
    ...
    def choosefile(self):
        ...
        if dlg.exec_():
            ...
            file_path = file_path.replace(filename, "")
            self.GetBtwList(r''+file_path)  
            self.node.set("fileurl","urlprod",file_path)
            print(file_path)
...
class OutSideCode(QWidget, Ui_OutSideCode, BoxSing):
    ...
    def __init__(self,parent=None):
        ...
        #加载已存在路径
        file_path = self.node.get("fileurl","urloutside")
        if file_path!="":            
            self.GetBtwList(r''+file_path)
    ...
    def choosefile(self):
        ...
        if dlg.exec_():
            ...
            file_path = file_path.replace(filename, "")
            self.GetBtwList(r''+file_path)
            self.node.set("fileurl","urloutside",file_path)
    ...
    def Printing(self):
        ...
        #==============获取项目代码===============
        project = dirn[5]
        pz = ms.get("select machine from machine where xm = '"+project+"'")
        machiname = list(pz)[0][0]
        
        print("project: "+project+"machiname: "+machiname)
       ...
       # print(counts)
        ...
        while num > 0:
        #==================================================================循环开始=====================================================================
            insql =  "INSERT INTO outcode(carton,date,piname,count,machine,optname,kongid,metaid,mode,hole,ischecked,waterid,project) VALUES "
            newcount = int(counts)+1
            count2 = newcount
            #获取新二维码序列
            newcount = self.ChangeNum(newcount)
            ...
            insql += "('"+newcode+"','"+dirn[2]+"','"+dirn[3]+"',"+str(dirn[4])+",'"+machiname+"','"+self.username+"','"+dirn[7]+"','"+dirn[8]+"','"+dirn[9]+"','"+dirn[10]+"','false','"+newcount+"','"+project+"')"
...
class LoginAdmin(QWidget, Ui_Admin, BoxSing):
    def __init__(self,parent=None):
        ...
        self.timer.timeout.connect(self.start)
        self.timer.start(1000)
        self.gouser.clicked.connect(self.hidewrap)
        self.limitword()
        self.loginbtn.clicked.connect(self.Login)
        self.password.returnPressed.connect(self.Login)
    def closewrap(self):
...
class CodeSet1(QWidget, Ui_CodeSet1, BoxSing):
    ...
    def loading(self):
        ...
        if db!=[]:            
            ...
            sql = "select machine from machine where xm='"+str(db[5])+"'"
            getmachine = list(self.ms.get(sql))
            #print(getmachine)
            if getmachine!=[]:
                machine = getmachine[0][0]
                self.machine.setText(machine)
        else:
            self.optname.setText(self.username)
            self.wei.setText("0")
            self.piname.setText("Cap")            
            print("暂无配置")
    ...
    def Cunchu(self):
        ...
        sql = "select xm from machine where machine='"+machine+"'"
        getmachine = list(self.ms.get(sql))
        if getmachine==[]:
            self.Alert("没有相应匹配的机机型")
            return False
        else:
            machine = getmachine[0][0]
        ...
        if len(code) == 18:
            ...
            sql = "select count(*) from outcodeset where 1=1"
            ishas = list(ms.get(sql))[0][0]
            if ishas==0:
                sql = "insert into outcodeset(code,dates,piname,count,machine,optname,kongid,metaid,mode,hole,waterid,head,wei)values("
                sql = sql + "'"+code+"','"+nowdate+"','"+piname+"','"+count+"','"+machine+"','"+optname+"','"+kongid+"','"+metaid+"','"+mode+"','"+hole+"','"+waterid+"','"+head+"',"+wei+")"
                print(sql)
                isupdate = ms.run(sql)
                if isupdate==True:
                    self.hide()
